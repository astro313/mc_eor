...
the simulation used here focuses on an LBG galaxy \citep{Pallottini17a} from the \ncode{serra} simulation suite 
...
part of \ncode{Krome}. % (see \citealt{Bovino16a}). 
We do not assume a clumping factor in the chemistry solver (cf. \citet[][]{Lupi20a}, i.e., $C_\rho$\eq1).
...
{\bf The stellar-to-halo mass ratio of \flower is $M_\star/M_{\rm DM}$\eq0.07.
Depending on the work considered, this ratio from abundance matching vary when the halo mass is away from $M_h$\ssim10$^{12}$\,\Msun (see e.g., \citealt{Moster13a, Behroozi13b, Behroozi15a, Moster18a}). The ratio of \flower is higher than e.g., \citet{Katz17a}  and \citet{Ceverino17a} which could be a result of different feedback implementation.
}
...
of the Milky Way but higher than what is typically observed in the Milky Way}, though the MCCs are more massive. {\bf The higher than typical MW value is linked to the higher turbulence of these systems.}
...
at the resolution of the simulation.}}. The virial parameter of these MCCs are, on average, lower than those identified at low $n_{\rm cut}$, and are thus, more unstable against collapse (see middle right panel of \Fig{alpha16-28}).
...
%
Close resemblance of the $Q_\star$ and $Q_{\rm eff}$ maps in the {\em central region} reflects the fact that stellar component plays an important role in governing the stability of MCCs against $m=0$  perturbations in {\bf this} relatively evolved and enriched systems at high redshifts.
\label{fig:Qeff}}
\end{figure*}
