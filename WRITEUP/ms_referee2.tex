...
{\bf dense gas clumps in which molecules form quickly (hereafter molecular-forming gas clumps\footnote{{\bf Note that by clumps, 
...
the gas {\bf (including thermal, turbulent, and bulk motions)}, $\mathcal{T}_{\rm ext}$ is external pressure support, and
...
\label{eqn:alpha_tot}
\end{equation}
where $M_\star$ is the stellar mass {\bf enclosed} within the MGC volume.
...
(see \citealt{Moster13a, Behroozi13b, Behroozi15a, Moster18a})\footnote{The stellar mass-to-halo mass ratio for Althaea at $z$\eq6 is about 0.07. 
...
We start by considering the distributions of MGC {\bf total gas mass $M_{\rm gas}$, radius $R$, and gas mass fraction}
\begin{equation}
...
of the Milky Way but higher than its average value}, though the MGCs are more massive. {\bf The higher than typical MW value is linked to the higher turbulence of these systems.}
...
%
{\bf While MGCs in both local ULIRGs and \flower may form via gravitational instability (see e.g., \citealt{McKee07a}), MGCs in the local merger-driven ULIRGs}
are likely formed by shock compression and cloud-cloud collisions that funnel large amounts of gas from the progenitor galaxies towards the central region (\citealt{Tan00a, Wu18a}; {\bf see \Sec{Qeff} for Toomre-$Q$ analysis on \flower}). {\bf As shown
