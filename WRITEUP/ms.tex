...
%mm, 8   [I only use the Drexel affiliation when working with folks
%from there...]
}}
...
%mm \altaffiltext{8}{Department of Physics, Disque Hall, Drexel University, Philadelphia, PA 19104, USA}
...
We use a clump-finding algorithm and a set of H$_2$ volumetric densities ($n_{\rm cut}$) 
to identify MCs and their sub-structures.
We extract properties such as mass, size, velocity dispersion, gas surface density, and virial parameter ($\alpha_{\rm vir}$) 
for each MC and 
...
The mass scale of the MCs is on the order of $10^{6.5-9}$\,\Msun. The $\sim$200\,pc-scale MCs found with a low density threshold 
...
with $\sigma\gtrsim$100\,\kms even when we increase the $n_{\rm cut}$ and even for  
the molecular substructures.
The high velocity dispersions likely result from 
the strong supernova and stellar feedback \flower experienced over the multiple episodes of bursty \SF.
Virial analysis indicates that the MCs found in the main disk of \flower are unbound, but its substructures have lower virial parameters. 
...
due to the less frequency \SF compared to \flower (also supported by the lower stellar-to-gas mass ratio of the former).
MCs in the satellites are therefore likely collapsing structures. This paints a picture, in which at the EoR, \SF continues 
as gas is being accreted from the intergalactic medium.
...
Our results are insensitive to the choice of $n_{\rm cut}$, except for the slope of the cumulative 
mass distribution, which steepens with $n_{\rm cut}$. 
...
the MC identified and compare them with observations of molecular clouds
and structures seen in nearby and \z$>$\,0 galaxies.
We present Toomre stability analysis in \Sec{Q}.
We discuss the results % , robustness of our analysis, caveats, 
and implications of our findings in \Sec{diss},
...
% {\AP potentially moving the following, maybe postpone after structures have been identified as a description? }
% {\DL or maybe in the discussion section?}
...
as examples in \Sec{singless} to illustrate the salient points of the discussion presented in this paper,
...
\begin{figure*}[htbp]
 \centering
  \includegraphics[scale=0.65]{\figpath/{dual_16_ncut_0.53}.pdf}
  \\ [-2.9em]
  \includegraphics[scale=0.65]{\figpath/{dual_16_ncut_6.81}.pdf}
  \\ [-2.9em] 
  \includegraphics[scale=0.65]{\figpath/{dual_16_ncut_18.96}.pdf}
\caption{
...
The plots shown in \Fig{MC} are weighted by the total gas density; that is, 
\begin{equation}
{\rm pixel_i} = \frac{\sum_i~\rho_{\rm H2, i}~w_i~\Delta z_i}{\sum_i~w_i~\Delta z_i}, 
\label{eqn:integrate}
\end{equation} 
...
\Fig{alpha16}). The V-shaped dashed lines in the figure show the loci along which the given external pressures are needed for 
...
the MC itself is gravitationally unbound and is supported
...
\begin{figure*}[htbp]
\centering
\includegraphics[trim=0 0 0 0, clip, width=0.65\textwidth]{\figpath/ss16-28_alphavir.png}
\includegraphics[trim=0 0 0 0, clip, width=0.65\textwidth]{\figpath/ss16-28_PVE.png}
\caption{
...
\begin{figure*}[htbp]
\centering
\includegraphics[trim=0 0 0 0, clip, width=0.65\textwidth]{\figpath/ss16-28-alphavir-highncut.png}
\includegraphics[trim=0 0 0 0, clip, width=0.65\textwidth]{\figpath/ss16-28-PVE-highncut.png}
\caption{
...
We show in \Fig{MR} the size-mass relation of the MCs of \flower in the accretion phase (see \Sec{sfh})
compared to the 
observational data of molecular clouds in the Milky Way that are found to be associated with massive \SF 
\citep{Beuther02a, Mueller02a, Hill05a, Motte07a} and the empirical relation obtained based on 
Milky Way data (\citealt{Kauffmann10b, Kauffmann10c} and references therein).
This relation is obtained by comparing the masses and sizes found in clouds with and without massive \SF ($<$10\,\Msun). 
\citet{Kauffmann10c} interpret this
mass-size relation of $M \gtrsim 870$\,\Msun$\times\,\left(r/{\rm pc}\right)^{1.33}$ as
the threshold for massive \SF.
The implication of this result is that cluster-forming cloud fragments are 
found to be more massive than those that are devoid of clusters 
at a given radius. 

As shown in \Fig{MR}, the MCs identified in the accretion phase of \flower lie above this relation, along the locus 
with a visual extinction of $A_V$\eq4\,mag \citep{Lombardi10a}, which 
% {\AP note that we can directly calculate Av given the dust properties in the galaxy or approximate it as av = cost N * Z con cost assuming a MW-like scaling}
is below the locus of $A_V$\eq100\,mag followed by Milky Way clouds.
...
fact that the MCs in our simulation have lower column density compared to the star-forming cores observed in the Milky Way, which are 
observationally well-resolved within an MC, and indeed, have sufficient column density to form CO.
% unresolved CO?detected molecular clouds in other galaxies should have an average A_V = 4 mag.

Similar MC properties in relation to those observed in the nearby Universe are found during the starburst phase of 
\flower (see e.g., bottom panel of \Fig{larsons_single} and \Fig{alpha27}).
On average, we do not find any quantitative differences in the MC properties in the accretion versus starburst phase.
...
\begin{figure}[htbp]
\centering
\includegraphics[trim=0 0 0 0, clip, width=0.5\textwidth]{\figpath/CMF_.png}
\caption{
CMF of MCs in \flower and best-fit power law.
\label{fig:cmf}}
\end{figure}

The mass range of the MCs identified in our simulation is $M_{\rm cl}\approx$10$^{6.5-9}$\,\Msun, which is
...
We find that 
the slope varies from $-$0.49\pmm0.01 to $-$1.17\pmm0.06 for the lowest and the highest $n_{\rm cut}$ adopted.
...
\DL{still yet to make the ``Q''-map}
...
\subsection{Variation in MC Properties Depending on the Choice of Density Cuts and 
...
in \Fig{alpha16-28}, \Fig{larsons16-28}, \Fig{alpha16-28-highncut},
and \Fig{larsons16-28-highncut}.
We find no quantitative differences in the cloud properties over the 700\,Myr traced in our simulation.
% \DL{This suggests that, at least ...., the ... of the cloud dynamics in \flower... over the timescale..}
...
substructures that are denser as we increase $n_{\rm cut}$ (see e.g., \Fig{larsons16-28} and \Fig{larsons16-28-highncut}). In any case,
the MCs we identified are much bigger in size and mass than nearby GMCs, which is consistent with
those observed in $z$\,\ssim2$-$4 galaxies based on spatially resolved imaging \citep{Swinbank11a}.
...
in gas-rich star-forming galaxies at $z$\ssim2 (see also e.g., \citealt{Swinbank11a}).
...
% would be interesting for the future, but is outside the aim of the present work
...
In M64 and M51, there are a few clouds with masses $M_{\rm cl}>$10$^7$\,\Msun \citep{Rosolowsky05a, Koda09a, Colombo14a}. 
On the other hand, such massive molecular structures are unknown to Local Group galaxies 
\citep[e.g.,][]{Rice16a}, and are likely the sites 
from which massive star clusters or OB associations are to be formed.
% 
The shallower slope found in the CMF for \flower compared to those observed in the nearby Universe (see \Sec{cmf}) may
...
That said, the Jeans mass $M_J$\eq$\sigma^4/(G^2\Sigma)$ yields $\sim$\,8\E{8}\,\Msun 
for a velocity dispersion of $\sigma\approx50$\,\kms and a gas mass surface density of 
$\Sigma_{\rm gas}\approx450$\,\Msun\,pc$^{-2}$, which are typical of the MCs of \flower.
This Jeans mass is in agreement with the mass range found (see \Fig{dist}). 

% CMF and IMF
The CMF is related to the mass function of stars (IMF),
since the former describes the mass distribution of 
the materials for \SF (see e.g., \citet{McKee07a} for a review).
Observational studies of a diverse range of environments suggest that the IMF is invariant 
between star-forming regions, star clusters, and spiral galaxies 
(see a review by \citealt{Bastian10a}; see also \citealt{Bochanski10a, AlvesdeOliveira13a, Calamida15b}), but is subject 
to variation under extreme conditions (e.g., temperatures, densities, and radiation field intensities), such as  
in the Galactic center (e.g., \citealt{Lu13a, Conroy13a}).  
Speculatively, the different CMF slope found in \flower may suggest a different IMF compared to those observed in nearby galaxies.
In particular, the steeper slope may indirectly translate into a top-heavy IMF (see \citealt{Alves07a, Anathpindika11a}), which is commonly assumed in most \highz studies to date\footnote{Since intuitively, we expect a higher Jeans mass at higher redshift, due to the lower metallicities and higher temperature
expected in these first galaxies.}. 
...
That said, one must acknowledge that such a zoom-in simulation 
is currently quite expensive to perform with 
existing computational power. %, given the complexity of the physics invoked as input.
...
%We show in \Fig{sk} the MCs of \flower identified in the accretion phase using different $n_{\rm cut}$
%(see \Sec{sfh} for details) and those reported in nearby and \highz \obs \citep{Kennicutt98a, Bouche07a, Daddi10a, Daddi10b, Genzel10a, Tacconi10a}. 
%
%
%\begin{figure*}[htbp]
%\centering
