...
as non-thermal pressure dominates thermal pressure for dense gas \citep{Pallottini17b}, i.e. $c_s^2 \ll \sigma_{\rm NT}^2$.
 ...
 % 
 Note that the bulk velocity dispersion $\sigma_{\rm bulk}$ is typically comparable to the non-thermal term $\sigma_{\rm NT}$, 
as the two are connected via the Euler equation, but this depends on the evolutionary stage of \flower (\Fig{vv}).
%
...
We note that \citet{Behrendt16a} similarly concluded that large scale bulk motion may mimic turbulent velocity dispersion 
due to beam smearing even though the self-rotation of each individual sub-MCCs is washed out.
