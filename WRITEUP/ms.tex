...
\caption{
    Star formation history of \flower (top right) and projected stellar mass distribution of \flower during {\it (a)} one of its accretion phases at its early stage of evolution; {\it (b)} one of its major starburst phases after a merger event; and {\it (c)} a relatively quiescent phase post-starburst.
\label{fig:SFH}}
\end{figure*}
...
The MCs in the satellite galaxies are found to follow the locus of $\log{(P/\textrm{K cm}^{-3})}$\eq6 (see \Fig{alpha27}), which 
...
Maps of the Toomre $Q$ parameters for the gas, stars, and the effective two-component $Q_{\rm eff}$ are shown in Figures~\ref{fig:Q}, \ref{fig:Qstar}, and \ref{fig:Qeff}, respectively (see also \Fig{h2density} for an example of MCs identified in this evolutionary stage of \flower
for different $n_{\rm cut}$). Fragmentation are expected in regions of low Toomre $Q$ values. \AP{why then we do not postpone the figure presentation?} \DL{I am not sure if I should first show all the plots in Sec 4 and the discuss in details in Sec 5 or what. I guess we could just show
in Sec 5.}
...
\caption{\AP{we should probably use horizontal colorbars or a signle vertical one, with gas stars and combined written as an inset; probably the 50 pc smoothing radius is be too large, we might want to go to 30 or 20}
%
}
\end{figure*}
