...
To visually display 
the clump finding procedure, we overplot the molecular structures identified using a subset of the H$_2$ density cuts ($n_{\rm cut}$\eq0.53, 5.81, and 18.96\,\cc) on the H$_2$ density maps (\Fig{MCC}).
...
c_s^2 = \frac{\sum_{i} \rho_i \left(P_i~k_B / M_p / {\bar \rho_i} \right)^2}{\sum_i \rho_i}\,,   % so that cs^2 has dimension of [erg/g]
\end{equation}
...
cloud identified at each density threshold (see top right corner of \Fig{stellarRatio16} and top left panel of \Fig{dist})  
...
The main disk occupying the top right corner of \Fig{stellarRatio16} has a stellar-to-gas mass ratio of $\simeq$\,50--100.
...
We first present results on the two most extreme evolutionary stages of \flower\ --- the accreting and starburst phases (see \Sec{sfh}).
Scaling relations for the MCCs identified are shown in \Fig{larsons_single} for the accreting (left panels) and starburst phase (right panels), respectively. 
...
\Fig{MCC} and \Fig{MCC28}, see also \Sec{distribution}), which includes a large fraction of stellar mass that are already assembled in \flower. 
...
% PVE
Motivated by observational studies, we also plot the $\sigma^2/R$ ratio and gas surface density of MCCs in the bottom left panels of \Fig{larsons_single}.
...
to have certain linewidths for a given set of surface densities (see \Sec{PVE}).
...
%the $M_{\rm cl}$-size relation is consistent with the extinction inferred from gas clumps analysing the UV and IR emission of \flower~by calculating the effect of radiative transfer through dust \citep[][in particular see Fig. 5 therein]{Behrens18a}
...
a wider range in gas surface density (bottom panels of \Fig{larsons_single}) and have lower virial parameters (thus, more susceptible to collapse).
...
for the virial parameter, where the most relevant phases are shown in \Fig{alphaEvol} (see \Sec{singless}).
...
and also resulting from the injection of kinetic energy from recent
...
\subsection{Stability of MCCs} \label{sec:Qeff}
