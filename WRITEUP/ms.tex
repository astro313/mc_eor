...
% is it what Jen's proposed? Or is it's just a result of $\rho\propto M R^{-3}$ (and $\rho$ is independent of R) 
% and observational bias (selection effect due to projection in 2D, so $\rho \propto N_H R^{-1}$, where $N_H$ is a 
% constant due to gas tracer) (i.e., middle and bottom panels of Fig 9 in BM02)?
...
We show the density-size relation ($\rho - R$) relation of MCCs of \flower in \Fig{rho}.
...
\caption{Density and size of MCCs in the accreting phase of \flower.
At higher $n_{\rm cut}$, MCCs are broken into smaller MCCs and 
form the cluster of points that are to the top left of the parent MCC identified at a lower density threshold.
\label{fig:rho}}
\vspace{0.5em}
\end{figure}
...
Their velocity dispersions are comparable to those observed in $z$\ssim2 starburst galaxies ($\sigma\simeq200$\,\kms); however, 
...
and \SF is expected to take place within its star-forming {\it clumps} and {\it cores} on smaller scales, 
where energy quickly dissipates. % with low virial parameters.
This is consistent with the notion that collapsing structures resulting from
...
we shown in this work that prototypical galaxies at the EoR with properties similar to \flower
...
large-scale motions even in the dispersion dominated stage. 
...
in galaxies at EoR. 
It is thus currently unclear whether \highz molecular gas structures are intrinsically more turbulent than those in the local ISM. 
This will become clearer in the next few years as deep and high resolution imaging of molecular gas at \highz are obtained with ALMA and NOEMA. 
