...
%mm (equivalent baryonic) 
mass resolution of $\simeq 6\times 10^4 \Msun$, 
%mm ($1.2 \times 10^4 \Msun$) [expanded out the meaning, but check if
%I have it right]  Such
...
%    local density, which would have to be specified here.} \AP{the setting description has been extended}
     Cell sizes are comparable to the sizes of local giant molecular clouds \citep[e.g.,][]{Sanders85a, Federrath13a, Goodman14a},
%mm 
%     \MM{Nota Bene!} \AP{This note is discussed in details in the following section. In the simulation we do not treat 5--10 cells as MCs, so I would remove the point from the present} although the resolved scale of 5--10 cells is rather larger.
...
$M_\star\simeq$\,3\E{10}\,\Msun, a metallicity of
$Z\simeq$\,0.5\,$Z_{\odot}$, a molecular gas mass of $M_{\rm
  H2}\simeq$\,5\E{7}\,\Msun, and a SFR of 30\,--\,80\,\Msun\,yr\pmOne. 
%mm \MM{What is its gas fraction?  I ask because its molecular gas
%mass is minuscule, particularly for such a high SFR: this is a
%depletion time of only 0.5 Myr, while modern galaxies usually have
%depletion times more like 0.5--2~Gyr!} \AP{the gas fraction is very
%flickering and depends on which radius we are considering, as the
%galaxy is actively accreating material and merging; depletion time
%are low, see the ssfr note added considering that $ssfr \propto
%t_{depletion}^{-1}$} 
...
\Fig{SFH}. The SFR of \flower varies between $\sim$30--80 \Msun\,yr\pmOne as it evolves from an actively accreting phase to a starburst phase after a merger, and then back to a relatively quiescent phase, over the simulated $\approx 700$ Myr.

Given the stochastic nature of \flower in its star formation history, we mostly focus on
two of its most extreme evolutionary stages in this work (see \Sec{singless}).
These phases correspond to (a) an intensely accreting phase and
(b) a starburst phase (\Fig{SFH}). We are interested in determining whether 
...
traced in the simulation (see \Sec{ncut}). 
In \Sec{rot}, we examine the importance of rotation support in the MCC dynamics and
discuss the implication of this finding in \Sec{rotDiss}.
...
the clump-finding algorithm available in the \ncode{python} package
\ncode{yt} \citep{Turk11a}, which was initially described in
\citet{Smith09a}, 
%mm but this function has been modified since then.
     and modified since.
The latest version of the default \ncode{yt} clump finder decomposes
the zones of the simulation into non-overlapping tiles, which are
stored in a 
%mm $k$-dimensional tree ($k$-D tree). [unused acronym]
...
%mm modify the function to enhance 
     enhanced
...
In the clump-finding process (in position-position-position space; PPP), we employ a set of different density
...
to identifying molecular structures based on the noise levels 
     in position-position-velocity (PPV) maps that
observers obtain with telescopes, using molecular line tracers such as
CO, CS, and HCN.
%mm , as commonly adopted in observational studies (e.g., 
...
\flower during its accreting phase, where \flower is dispersion-dominated.
...
In \Fig{h2density}, we show the distribution of H$_2$
...
To visually display 
the clump finding procedure, in \Fig{MCC} we overplot the molecular structures identified using a subset of the H$_2$ density cuts ($n_{\rm cut}$\eq0.53, 5.81, and 18.96\,\cc) on the H$_2$ density maps.
...
For instance, the bulk is calculated as follows
\begin{equation}
...
c_s^2 = \frac{\sum_{i} \rho_i \left(P_i~k_B / M_p / <\rho_i> \right)^2}{\sum_i \rho_i}\,,   % so that cs^2 has dimension of [erg/g]
\end{equation}
...
% allss: 0.32 
% In [13]: print(get_Mach_all_clouds(ss11))
%[10.62623949  3.10713402  7.0525831   3.336829   10.36052827  7.24157821
%  7.12323137  6.75282955  9.94086767  7.47769907  3.39894958  3.27304956
%  6.69290803 11.44844494  7.03862716  3.16574077  5.1015787   5.67749554
%  7.35946615  6.89475134  7.2433421  10.57019778 11.78414139  3.93499799
%  6.86173652  7.39686112  8.37197686  6.71749578  2.65304172  6.49617895
%  5.15095491  5.6606244   2.98923849  3.65475035  7.37703114  4.5669722
%  7.18720926  7.35017082  6.03000248  6.36373235 10.16701766  8.3894084
%  3.09284034  4.37994321  4.46497953  5.264101    8.11686     3.05183157
%  7.10365783  6.09513017  8.72517656  5.59659665  3.95290202  9.22726672
%  4.94960657  6.19757621  4.63335759  3.32670022  3.46706217  5.58358069
%  3.41808098  8.31386962  4.26357159  3.684986    4.01853538  3.62917357
%  6.05586465  9.82127908  6.68366736 11.0791874  13.28965621 10.42021747
%  3.04409362  7.21509021  3.63699627  8.12503492  3.20158298]
% allss: 18.96
%In [15]: print(get_Mach_all_clouds(ss12))
%[15.25049689  6.74580503  6.98246499  5.17070655  6.470997    7.02626558
%  4.42663778  6.34460542  4.90603266  3.66900884  4.03260414  6.9539893
%  8.36247744  4.89164338  4.12695121  6.55187227  6.15391173  3.56580625
%  5.74818066  3.81067175  6.65393948  9.10100946  6.35039889  4.78667156
%  5.05517168  6.93886988  5.4830306 ]
but this depends on the evolutionary stage of \flower.  % see also e.g., test_brute/leaf_fields_ss16-28/18.96_10_fields/Mach_cloud-mass.png
% This is consistent with the global analysis done on \flower by \citet{Vallini18a}.


The highest density threshold of $n_{\rm cut}$\eq31.62\,\cc corresponds to 
...
we therefore consider MCCs identified with the second highest density threshold ($n_{\rm cut}$\eq18.96\,\cc) in the following. 
...
clouds identified at low density cut (see top left panel of \Fig{dist})  
...
MCCs identified across all evolutionary stages of \flower (see e.g., \Fig{stellarRatio16}).
...
\caption{Distributions of mass (left), size (middle), and gas mass
  ...
  (top panels) and $n_{\rm ncut}$\eq18.96\,\cc (bottom panels)
...
\caption{
Size-mass relation of MCCs identified in the accreting phase of \flower in our simulation (star symbols) compared to observational data of molecular clouds in the Milky Way associated with massive \SF (magenta circles, green stars, blue dots, and black triangles) and empirical relations established based on \obs of the Milky Way. Red line shows the 
mass-size relation found for regions in the Milky Way with massive \SF
reported by \citet{Kauffmann10b}. Star symbols are color-coded by
...
\begin{figure}[htbp]
\centering
\includegraphics[trim=0 0 0 0, clip, width=0.5\textwidth]{\figpath/Mach_pressure-stellar-to-gas_ss21-ss27.png}
\caption{
...
\DL{replace w/ non-thermal velocity versus bulk velocity plot to investigate importance of differential rotation in the derived velocity dispersion.}
...
We present results on the two most extreme evolutionary stages of \flower\ --- the accreting and starburst phases (see \Sec{sfh}), 
and identify a set of MCCs in each of them. 
Scaling relations for the MCCs are shown in \Fig{larsons_single} for the accreting (left panels) and starburst phase (right panels), respectively. 
...
% Kennicutt, 1996IAUS..171...11K
% Spiral gal, appears to have accreted a gas-rich companion in the recent past, 
% with the inner gas disk the remnant of the
% original disk (Braun et al. 1992; Rubin 1994; Rix et al. 1995). 
the $z$\ssim2 starbursting disk galaxy SMM\,J2135-0102 % SFR: 400, fgas ~ 0.6.
...
mainly composed of molecular gas only, we calculate the virial parameter including the influence from the stellar component 
for MCCs of \flower (i.e., $\alpha_{\rm vir, tot}$ via Equation~\ref{eqn:alpha_tot}) given the high stellar-to-gas mass ratios of 
some of them (see \Fig{stellarRatio16}). 
MCCs with the highest stellar-to-gas mass ratio ($\gtrsim$\,1) correspond to the central main disk of \flower (see top panels of 
\Fig{MCC} and \Sec{distribution}), which includes a large fraction of stellar mass that are already assembled in \flower. 
This explains the low total virial parameter seen in the most massive MCCs of \flower in this phase, as shown in
the middle left panel of \Fig{larsons_single}, where the stellar gravitational potential influence the overall stability of such MCCs.

% PVE
Motivated by observational studies, we also plot the $\sigma^2/R$ ratio and gas surface density of MCCs in the bottom left panel of \Fig{larsons_single}.
...
linewidths for a given set of surface densities (see \Sec{PVE}).
...
scales via mass and size \citep{Larson81a,
  McKee07a}. Since cloud mass in observational studies is sometimes derived by integrating over the mass surface density, 
which is related to the column density ($N_H$) that is obtainable from extinction maps ($A_V$), the mass-size relation is 
also known as the constant column density relation. 
\MM{And is thus inconsistent with the variation in $\sigma^2/R$ with
  ...
  \DL{will discuss implication in \Sec{MR}.}
...
As shown in \Fig{alphaEvol}, the total virial parameter (blue dots in the figure) 
...
%Similarly, the stellar-to-gas mass ratio of MCCs in the starburst phase is higher overall (see bottom panels of \Fig{alpha21-22}).
%These trends are as expected in the MCC properties prior to and during a starburst. 
% None of these MCCs are in the unstable regime ($alpha_{\rm vir} < 2$) because the MCCs are not directly the collapsing structures, 
% i.e., we haven’t reached those scales where true collapse is happening. 
...
As shown in \Fig{Mach_stellarRatio}, the Mach number (see Equation~\ref{eqn:MPress}) 
of MCCs with a stellar-to-gas mass ratio of $M_\star/M_{\rm gas} \simeq 0.1 - 1$ 
(i.e., excluding the main component of \flower with $M_\star/M_{\rm gas} \gtrsim 1$; see e.g., \Fig{stellarRatio16})
...
the results presented in \Sec{singless} dependent on the choice of density thresholds.
...
In this way, we identify a population of denser molecular structures (see \Fig{MCC}). 
...
% Also mentioned in \Sec{sfh}, we will frame the remainder of this work focusing on the properties of MCCs identified in the two most extreme 
% evolutionary stages of \flower.
...
% 
2) Comment somewhere that feedback plays in important role in the virial parameter, as 
can be seen in the strong variation in alphavir across the different evolutionary stages of \flower \Fig{alphaEvol}.
% 
...
linewidth-size relation of \Fig{larsons_single} and the top left panel of \Fig{alpha16-28}), which correspond to 
the main disk of \flower, the MCCs of \flower have dynamics largely similar to those observed in
$z$\ssim2 spatially resolved studies of gas-rich star-forming
galaxies, in terms of their velocity dispersions, sizes, and gas
surface densities (\Fig{larsons_single}; see e.g.,
...
episodes of \SF as \flower is assembling its stellar mass --- these MCCs have higher stellar-to-gas mass ratios of $M_\star/M_{\rm gas}\simeq0.1 - 1$ (\Fig{stellarRatio16}) and high turbulent pressure as shown by the high $\mathcal{M}$ in \Fig{Mach_stellarRatio}.  
%
In fact, by the accreting stage (see \Fig{SFH}a) at $z\simeq$\,7.2, 
\flower has assembled a stellar mass of
$M_\star$\eq7.5\E{9}\,\Msun. 

The high pressure observed in \flower (see \Fig{Mach_stellarRatio}, see also \citealt{Pallottini17a})
...
%mm they are likely form 
  MCCs are likely formed
by shock compression and cloud-cloud collisions 
%mm after large amounts of gas from the pair of progenitor galaxies
%are funneled toward the central region 
...
in gas-rich star-forming galaxies at $z$\ssim2 (see also e.g.,
...
main galaxy 
%\MM{This is a highly dissipative process.  Hennebelle \&
...
%  inter-cloud collisions being important?}
%\DL{think okay to state as speculation since we are not drawing conclusions based on this?} 
...
between the accreting phase and the starburst phase of \flower in terms of the Larson's relation (see
\Fig{larsons_single}), which are separated by $\simeq$300\,Myr. 
However, MCCs in the pre-\SB phase have lower $\alpha_{\rm vir}$ and pressure compared to the starburst phase. 
Similarly, we see that MCCs .... in the post-starburst phase. 
This is expected for \SF.

\subsection{Importance of Bulk Motions in Measured Velocity Dispersion}
...
\caption{Velocity dispersions from bulk motions and turbulent motions of \flower in accreting phase (top) and 
...
Most MCCs in the accreting phase have velocity dispersions from turbulent motions higher than those from bulk motions. 
The opposite trend is true in the quiescent phase.
\label{fig:vv}}
\end{figure}

We compare the velocity dispersion from bulk motions versus turbulent motions in \Fig{vv}, for the accreting phase
and the quiescent phase. The accreting phase has a disturbed morphology while the quiescent phase 
displays a disk-like morphology (see \Fig{MCC} and \Fig{MCC28}). 
Most MCCs in the accreting phase have turbulent velocity dispersions higher than the bulk motion velocity dispersions. The opposite is true for the 
quiescent phase. Second, the two velocity dispersions are of the same order of magnitude. 
This indicates that the .... have an important contribution from large-scale rotation.
\DL{ rewrite: This also poses challenge to measurements and interpretation of .... from existing observations of 
molecular ``clouds'' observed with 
ALMA and VLA. 
n, and so comparison of their dynamics with local clouds via e.g., Larsons relations is not straightforward, and thus, the interpretation of such. 
Meaning a lot of the sigma people infer at the sub-pc scale have a non-negligible contribution from rotation (cf. solely random motion), so it's not immediately fair for observations to say that higher redshift molecular gas are more turbulent based on solely measured velocity dispersion. 
}



\subsection{Interpretation of Mass-Size Relation} \label{sec:MR}
\DL{To be rewritten later.}
...
%
That is, the MCCs of \flower lying along the locus of $A_V$\eq4\,mag 
%mm likely manifests from the fact 
    demonstrates
...
denser as we increase $n_{\rm cut}$ (see \Sec{ncut}). Overall, the MCCs we identified are much bigger in
size and mass than nearby giant molecular clouds (GMCs), but comparable to those observed in $z$\,\ssim2$-$4 galaxies based on spatially resolved imaging \citep{Swinbank11a}.

% mass scale
Observations of nearby gas-rich galaxies such as M64 and NGC\,253 show
higher velocity dispersions compared to the disk
%mm/mid-plane 
of the Milky Way but consistent with those observed in the inner regions of Milky Way and M51 \citep{Oka01a, Rosolowsky05a, Heyer09a, Hughes13b, Leroy15a, Rice16a}.  % also higher $P_{\rm int}$,
...
%mm a result of their higher gas mass fractions, [I had trouble
%interpreting this sentence. Do you agree with the revision?]
    forming because of the higher gas fractions in these galaxies,
    which lead to higher
surface densities, and 
%mm velocity dispersions
...
$M_J\sim$\,8\E{8}\,\Msun, which is in good agreement with the mass
range found in the MCCs identified (see \Fig{dist}).  \MM{I really
...
%
Similarly massive \MM{8\E{8} \Msun?} molecular clouds have been reported in idealized closed-box isolated galaxy simulations done at higher resolution (e.g., a maximum
...
virial parameters, \MM{This needs to be shown explicitly in a figure,
  as I think I already mentioned above.} which are comparable to those found in clouds formed in the galactic disks  ($\alpha_{\rm vir}$\eq0.2$-$10; \citealt{Dobbs08a, Tasker09a}).
...
Some MCCs \MM{But these are the exception.} lie in regions of $\log{Q_{\rm eff}}\gtrsim0$, where regions of $\log{Q_{\rm eff}}\lesssim0$
...
\MM{Well, there are definitely regions dominated by gas.}  highlighting the importance of accounting for stellar contribution when examining the stability of molecular gas in relatively evolved and enriched systems at high redshifts.
\label{fig:Qeff}}
\end{figure*}
...
\subsection{Importance of Rotation in Molecular Structures Observed To-date} \label{sec:rotDiss}
read paper on angular momentum of clouds in \citep{Jappsen04a}.

How important is rotation energy in supporting the MCCs we see in \flower, versus pressure supported? Do we see differences 
in the dynamical properties of MCCs ($\sigma_{\rm pressure, NT}$ vs. $\sigma_v$) between ss16 and ss28? 


Concerning the role of rotation from large-scale motion in contributing to the observed linewidths of molecular gas structures identified in spatially resolved \obs of \highz galaxies.
poses a challenge to the interpretation of existing \obs of spatially resolved molecular gas structures detected in \highz galaxies to-date. 
...
\DL{Note to self: New observations that are now routinely obtained with ALMA provides 
sensitive and high fidelity images revealing the morphology of gas and dust in \highz galaxies, and possibly on sub-kpc scales. However, 
as shown here, prototypical galaxies at such high redshift with properties similar to \flower\ will have molecular gas structures
on sub-kpc scales, but they are not the truly star-forming clumps. As illustrated by studies in local galaxies, \SF proceed on smaller scales than 
what existing \obs are reaching. While the properties of galaxies at EoR differ from nearby galaxies, we argue that 
the physics behind \SF is the same, and so regions of where \SF truly takes places/unstable to collapse is on even smaller scales. 
This results means that caution should be taken when interpreting observational data, albeit the impressive resolution and sensitivity that are unprecedented. 
Careful consideration of large-scale motions is required when interpreting the observed data in order to understand the origin of star formation activity 
in galaxies at EoR. 
}
