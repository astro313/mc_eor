...
%
In particular, it uses a modified version of \ncode{ramses} \citep{Teyssier02a} as the AMR backend. The simulation used here covers a comoving box of 20\,Mpc $h$\pmOne in size. The simulation zoom-in on a target halo of mass $\sim 10^11 \Msun$. The Lagrangian region of the halo (2.1\,Mpc $h$\pmOne) has a dark matter (equivalent baryonic) mass resolution is $\simeq 6\times 10^4 \Msun$ ($1.2 \times 10^4 \Msun$). Such region is spatially refined with a quasi-Lagrangian mass threshold-based criterion, i.e. a cell is refined if its total (DM+baryonic) mass exceed the the mass resolution by a factor 8. The final finest refined cell in the zoom in region has size $l_{\rm cell}\simeq$\,30\,pc (at $z = 6$).
%mm
...
%mm including ${\rm e}^{-}, \rm{H}$, $\rm{H}^{+}$, $\rm{H}^{-}$, $\rm{He}$, $\rm{He}^{+}$, $\rm{He}^{++}$, $\rm{H}_2$, $\rm{H}_2^{+}$ 
   following e$^{-}$, H$^+$, H$^-$, He, He$^+$, He$^{++}$, H$_2$, and H$_2^+$ \citep{Grassi14a,Bovino16a}. Of particular importance here is that abundances are calculated using an on-the-fly non-equilibrium formation of molecular hydrogen scheme 
...
%mm
   \MM{confirm:} Photoionization from local sources is neglected in this simulation \citep[see][for its effect]{pallottini:2019,decataldo:2019}. \AP{modified accordingly}
...
that at $z\simeq 6$ is hosted by a dark matter halo of mass $M_{\rm DM}\simeq$\,10$^{11}$\,\Msun at the center of a cosmic web knot, and accretes mass from the IGM mainly via three filaments of length $\simeq$\,100\,kpc. \flower has a stellar mass of $M_\star\simeq$\,3\E{10}\,\Msun, a metallicity of $Z\simeq$\,0.5\,$Z_{\odot}$, a molecular gas mass of $M_{\rm H2}\simeq$\,5\E{7}\,\Msun, and a SFR of $\simeq$\,100\,\Msun\,yr\pmOne. \MM{What is its gas fraction?  I ask because its molecular gas mass is minuscule, particularly for such a high SFR: this is a depletion time of only 0.5 Myr, while modern galaxies usually have depletion times more like 0.5--2~Gyr!} \AP{the gas fraction is very flickering and depends on which radius we are considering, as the galaxy is actively accreating material and merging; depletion time are low, see the ssfr note added considering that $ssfr \propto t_{depletion}^{-1}$} The specific SFR of \flower is ${\rm sSFR} \equiv {\rm SFR}/M_{\star} \simeq 4 -- 40 \rm Gry ^{-1}$, compatible with sSFR vs $M_{\star}$ relation observed at high-redshift \citep{Jiang2016}, that is higher with respect to normal local galaxies. Its effective stellar radius  is $\sim$0.5\,kpc and the dark matter virial radius is $r_{\rm 200}\simeq$\,15\,kpc.
