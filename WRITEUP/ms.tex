...
systematically higher than those observed in nearby galaxies but comparable to 
starburst galaxies, with $\sigma\gtrsim$\,100\,\kms.
% Toomre Q
...
We further perform virial analysis, which is observable, to examine collapsing regions.
Virial analysis indicates that the MCs in the main disk of \flower are unbound, but its 
substructures have lower virial parameters,
...
which we interpret as result from the weaker stellar feedback
due to the less frequent \SF compared to \flower.
...
will furnish scrutinizing data to test our findings and shed light on \SF since the cosmic dark ages.
\end{abstract}
...
inhibit collapse even on these scales. Thus, a disk is unstable in the regime of $L_J < L < L_{\rm rot}$, where $L_{\rm rot}$ is set 
% \propto \Sigma/B^2
...
we consider the approximation for the effective two-component $Q_{\rm eff}$ parameter following \citet{Romeo11a, Inoue16a}
...
T = \[ \left\{
		\begin{array}{lccr}
			{\displaystyle 0.8 + 0.7\left(\frac{\sigma_{z}}{\sigma_{r}}\right)}      && & \mbox{if\ } \sigma_z \lesssim 0.5 \times \sigma_r \\ [0.75em]
			{\dispalystyle 1 + 0.6 \left(\frac{\sigma_z}{\sigam_r}\right)}             & & & \mbox{if\ } \sigma_z \gtrsim 0.5 \times \sigma_r 
\\
		\end{array}
		\right.
		\]
...
In next section, we examine the MC properties in the context of the Larson's relations \citep{Larson81a}, which
...
Maps of the Toomre Q parameter of the gas component are shown in \Fig{Q} for 
different projection angles during the accretion phase of \flower (see \Fig{h2density} for examples of clumps identified). 
...
%Observationally, virial parameters of clouds form in the galactic disk
%are found to have $\alpha_{\rm vir}$\eq0.2$-$10 \citep{Dobbs08a, Tasker09a}.
...
Thus, \SF continues as gas from the satellite galaxies is being accreted onto the main galaxies during the EoR.
...
substructures that are denser as we increase $n_{\rm cut}$ (see e.g., \Fig{larsons16-28}). In any case,
...
%% pressure
%The pressure of an MC is expected to increase with its velocity dispersion and surface density (see e.g., \Eq{pext} and
...
%The higher $\Sigma_{\rm gas}$ is in turn relates to more massive clouds being formed.
%In fact, a few clouds with masses $M_{\rm cl}>$10$^7$\,\Msun 
%are found in nearby gas-rich galaxies such as M64 and M51 \citep{Rosolowsky05a, Koda09a, Colombo14a}.
