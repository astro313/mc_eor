...
\begin{abstract}
We study the dynamical properties of molecular cloud complexes (MC) in a prototypical galaxy (\flower) and its satellite galaxies at the Epoch of Reionization (EoR) using cosmological zoom-in simulation.
%
...
%
The MCs span a mass range of $10^{6.5-9}$\,\Msun. Their velocity dispersions and gas surface densities are systematically higher than those observed in nearby galaxies, but comparable to starburst galaxies.
%
We compute the effective two-component Toomre $Q$ parameter (gas and stars) to examine regions that are 
gravitationally unstable.
%
We further perform virial analysis, which is accessible from observations, to examine regions likely 
to undergo collapse. Our analysis indicates that the MCs in the main disk of \flower are unbound, but its substructures have lower virial parameters and Toomre $Q$, which is consistent with the notion that collapsing structures occur within globally stable structures.
%
We find $\alpha_{\rm vir}\approx$\,2 for the MCs in the satellite galaxies, which we interpret as result of the weaker stellar feedback 
compared to \flower. Our results suggest that the more massive and bigger MCs compared to the Milky Way manifest from the 
...
%
We find $\sim$10$-$13 MCs with masses exceeding 10$^8$\,\Msun across all the considered evolutionary stages for each of the given $n_{\rm cut}$ (see \Sec{method}). The mass of the MCs identified ranges between $M_{\rm cl}\approx$10$^{6.5-9}$\,\Msun, consistent with those observed in \z$\sim$2 galaxies in rest-frame {\it UV and optical light} \citep{Elmegreen07a, Elmegreen09a}.

\begin{figure*}[htbp]
\centering
\includegraphics[trim=0 0 0 0, clip, width=0.7\textwidth]{\figpath/minmaxNcut_basicDistributions.png}
\caption{Distributions of mass, size, and gas mass fraction of MCs identified using the lowest $n_{\rm cut}$ (left panels) and $n_{\rm ncut}$\eq18.96\,\cc (right panels).
%
...
\label{eqn:v0}
\end{equation}
Therefore, for a virialized cloud, one expects a one-to-one mapping between $V_0^2$ and $\Sigma$ since $\sigma^2/R\propto\Sigma$, whereas Equation~\ref{eqn:v0} represents the locus along which external pressures $P_{\rm ext}$ are needed in order for MCs to have linewidths $\sigma$ for a given set of surface densities. % (see\Fig{alpha27}).
...
\label{eqn:Q}
\end{equation}
In the thin disk approximation ($kh\ll1$), the solution of \Eq{3Ddisp} is unstable for scales $k$ such that $Q < 1$ (or equivalently $\omega^2 < 0$). Physically, this situation happens when local gravitational instability 
overcomes support from pressure and rotation, allowing fragmentation and collapse to happen.
% but further evolution depends on their EOS. (e.g., instability impose by angular momentum from gas accretion from the cosmic web and satellite galaxies and stellar feedback).
% Note that fragmentation is expected to happen at critical scale length $\lambda_{\rm crit} < 2 \pi^2 G \Sigma / \kappa^2$, which is greater than the typical size of  GMCs. It should be clear that the instability sets the scales for fragmentation, but star-forming GMCs are the collapsing, denser, and cooler molecular structures on even smaller scales. The fragmented structures thus correspond to regions where $Q\lesssim1.
...
For the purpose of this work, we present Toomre $Q$ map in \Sec{results} and discuss the importance of support from galactic rotation and shear against axisymmetric perturbations in \Sec{origin}.
...
%
Scaling relations for the MCs identified in the accretion phase are shown in top panels of \Fig{larsons_single}, \Fig{alpha16}, and \Fig{alpha27}, and in the bottom panels for the starburst phase.
%
Regardless of the H$_2$ density cuts adopted, the MCs are found to have consistently \AP{which is already said in the last part of previous Sec.; I would put here that part} high turbulences and surface densities in both stages. These correspond to MCs at the center of the main disk of \flower and occupy the top right corner of the Larson's relation shown in \Fig{larsons_single}.
%
...
% PVE
We also compare the MCs identified in our simulations to those observed in the Milky Way in the context of the $\sigma^2/R - \Sigma$ relation (see \Fig{alpha27}). The dashed lines in the figure show the loci along which the given external pressures are needed for MCs to have linewidth for a given set of surface densities (see \Sec{PVE}).
...
The MCs in the \AP{i do not find a clear introduction of satellite galaxies in the text} satellite galaxies are found to follow the locus of $\log{(P/\textrm{K cm}^{-3})}$\eq6 (see \Fig{alpha27}) \AP{note that such numbers are consistet with the bulk of dense gas found in the simulation in the phase-space analysis in \citet{Pallottini17a}}.
...
Maps of the Toomre $Q$ parameters for the gas, stars, and the effective two-component $Q_{\rm eff}$ are shown in Figures~\ref{fig:Q}, \ref{fig:Qstar}, and \ref{fig:Qeff}, respectively (see \Fig{h2density} for an example of MCs identified  in this evolutionary stage). Fragmentation are expected in regions of low Toomre $Q$ values. \AP{why then we do not postpone the figure presentation?} Further interpretation and implication of this is discussed in \Sec{origin}.

\begin{figure*}
\centering
\includegraphics[width=0.9\textwidth]{\figpath/{ss16_toomre_combined_2by2_0}.png}
\caption{\AP{let me know which scheme you prefer (as per mail) or you would like a different plotting scheme; in any case, i guess we should go from $\log \sigma$ to $\sigma$; additionally we might want to use shared colorbars (e.g. $\Sigma_{gas}\,,\Sigma_{\star}-> \Sigma$) and write gas and stars as white text on the maps}
%
}
\end{figure*}
...
\begin{figure*}
\centering
\includegraphics[width=0.9\textwidth]{\figpath/{ss16_toomre_combined_3by1_0}.png}
\caption{\AP{if we keep a 3x1 scheme we should probably use horizontal colorbars or a signle vertical one, with gas stars and combined written as an inset; probably the 50 pc smoothing radius is be too large, we might want to go to 30 or 20}
%
}
\end{figure*}
...
%
In fact, by $z\simeq$\,7.2 (\AP{i would always refer to the evolutionary stage name, once in defined in Fig. 1; we might want to use italic to underline it is a definition} i.e., the evolutionary stage corresponding to an early phase of accretion as shown in \Fig{SFH}a), \flower has assembled a stellar mass of $M_*$\eq7.5\E{9}\,\Msun. Thus, the higher velocity dispersion is due in part to the stronger stellar feedback compared to e.g., MCs in its satellite galaxies ($\sigma\approx$10\,\kms).
...
%
That is, the MCs of \flower lying along the locus of $A_V$\eq4\,mag likely manifests from the fact that the MCs in our simulation have lower column densities than the star-forming cores observed in the Milky Way, which are observationally well-resolved within an MC, and indeed, have sufficient column density to form CO \AP{this AV is also consistent with the result reported in \citet{Behrens18a}}.

% different evolutionary stages
In the scaling relations examined, we find similar MC properties in relation to those observed in the nearby Universe between the accretion phase and the starburst phase of \flower (see \Fig{larsons_single}, \Fig{alpha16}, and \Fig{alpha27}). That is, we do not find any quantitative major differences in the MC properties between the accretion and the starburst phase, which are separated by $\simeq$300\,Myr.

\subsection{Origin and Physical Scales of the MCs} \label{sec:origin}

The largest molecular structure identified is essentially the main disk of \flower and breaks down into smaller substructures that are denser as we increase $n_{\rm cut}$ (see e.g., \Fig{larsons16-28}). Overall, the MCs we identified are much bigger in size and mass than nearby GMCs, but comparable to those observed in $z$\,\ssim2$-$4 galaxies based on spatially resolved imaging \citep{Swinbank11a}.
...
Based on the scaling of $M_J$\eq$\sigma^4/(G^2\Sigma)$ for the Jeans mass, a velocity dispersion of $\sigma\approx50$\,\kms and a gas mass surface density of $\Sigma_{\rm gas}\approx450$\,\Msun\,pc$^{-2}$ yield $M_J\sim$\,8\E{8}\,\Msun, which is in good agreement with the mass range found in the MCs identified (see \Fig{dist}).
%
...
% alpha parameters
As shown in \Fig{alpha16}, the virial parameter of the sub-MCs is about $\alpha_{\rm vir}\gtrsim10$, which is considerably lower than their ``parent'' MCs found in the main disk of \flower ($\alpha_{\rm vir}\gtrsim100$).
%
...
For MCs in the satellite galaxies, we find low pressures and virial parameters  which are comparable to those found in clouds formed in the galactic disks  ($\alpha_{\rm vir}$\eq0.2$-$10; \citealt{Dobbs08a, Tasker09a}).
