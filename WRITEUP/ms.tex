...
to \obs.
%
In \flower, the typical MCC mass is $10^{6.5}$\,\Msun, with sizes $R\lesssim100$\,pc.
%
...
%
We show that large-scale rotation is an important contributor to the velocity dispersion of some MCCs.
...
%
Their velocity dispersions are also comparable to those observed in the inner Milky way and nearby gas-rich galaxies (e.g., M64; \citealt{Oka01a, Rosolowsky05a, Heyer09a}), which lies along the locus of $\sigma_{\rm gas}\propto R^{0.56}$. Such high velocity dispersion is dominated by non-thermal energy (\Fig{vv}) and results from the injection of kinetic energy from recent \SF as \flower is assembling its stellar mass (given the high Mach numbers of MCCs; \Sec{distribution}). In fact, by the accreting stage (see \Fig{SFH}a) at $z\simeq$\,7.2, \flower has assembled a stellar mass of $M_\star$\eq7.5\E{9}\,\Msun.
...
%
First, due to energy dissipation on small-scales, such (sub-)structures are no longer supported by turbulence. 
...
Excluding the biggest MCC, the typical mass of MCCs is $10^{6.5}$\,\Msun and are supported by 
non-thermal pressure. Similarly massive molecular structures (few times 10$^7$\,\Msun) have been
...
owever, the IGM and merger and accretion histories are not properly modeled in such simulations.
...
traced in our simulation. One of the most prominent differences is the virial parameter, more MCCs have lower virial parameters
in the starburst phase compared to the accreting phase of \flower.
