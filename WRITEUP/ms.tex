...
In this section, we compare the observable quantities of MCCs obtained from the simulations (e.g., $\sigma_{\rm gas}$, $R$, $M_{\rm gas}$,
$\Sigma_{\rm gas}$) with those of molecular gas structures from \obs at.
...
and velocity dispersions of $\sigma_{\rm gas}\simeq$\,20$-$80\,\kms.
...
locus of $\sigma_{\rm gas}\propto R^{0.56}$.
...
\SF as \flower is assembling its stellar mass (given the high Mach numbers of MCCs; \Sec{distribution}).
In fact, by the accreting stage (see \Fig{SFH}a) at $z\simeq$\,7.2,
\flower has assembled a stellar mass of
$M_\star$\eq7.5\E{9}\,\Msun.
Furthermore, a comparison of the velocity dispersion from bulk motions versus that from turbulence (\Fig{vv}) suggests that
...
interpreting observational results is further discussed in the next subsection (\Sec{rot}).
...
% Tasker09a
On the other hand, the highly turbulent structures of \flower are the result of extra-planar flows \citep{kohandel:2019} and higher velocity accretion/SN-driven outflows \citep{gallerani:2018}. The presence of extra-planar flows may also be the dominant mode for forming the highly supersonic massive MCCs observed in gas-rich star-forming galaxies at $z$\ssim2 (see also e.g., \citealt{Swinbank11a}) --- but the observed size scales could also be biased high due to the resolution limit.
...
% see also e.g., test_brute/leaf_fields_ss16-28/18.96_10_fields/Mach_cloud-mass.png
% This is consistent with the global analysis done on \flower by \citet{Vallini18a}.


We show the gas velocity dispersions from bulk motions versus turbulent motions for MCCs in the accreting
and quiescent phase in \Fig{vv}. We note that \flower displays a disturbed morphology in the accreting phase
but a disk-like morphology in the quiescent phase
(see \Fig{MCC} and \Fig{MCC28}).
Most MCCs in the accreting phase have turbulent velocity dispersions higher than the bulk motion velocity dispersions,
whereas the opposite is true in the quiescent phase. 
In addition, the two velocity dispersion terms are of the same order of magnitude. % as the two are connected via the Euler equation
...
\citet{Behrendt16a} draw a similar conclusion using pc-scale resolution simulation: 
large-scale bulk motions may mimic turbulent velocity dispersion due to  
beam smearing, even though contributions due to self-rotation of each individual sub-MCCs is washed out within the beam.
...
highlighting the importance of accounting for stellar contribution when examining the stability of molecular gas in relatively evolved and enriched systems at high redshifts.
\label{fig:Qeff}}
\end{figure*}
...
has a scale height-to-radius ratio of $h/r_{\rm gal}$\ssim150\,pc/1\,kpc\,$\simeq$\,0.15.

That is, some MCCs are found in regions of $Q_{\rm gas}$\ssim1, which is consistent with the expectation that
...
% Mechanisms causing non-axisymmetric perturbations (e.g.,
...
%   subject (2011, doi:10.1017/S1743921311000640), so this isn't really a separate mechanism.}
% can induce orbit crossing, shocks and dissipation in gas, promoting turbulence dissipation and collapse on smaller scales to trigger \SF. These mechanisms are likely important given high $Q_{\rm eff}$ found across \flower and its few tens of solar masses per year SFR, albeit sporadic.
