...
As shown in \Fig{alpha16}, their virial parameter ($\alpha_{\rm vir}\gtrsim10$)
...
In fact, based on the virial parameter, the MC identified in (the densest region of) the main disk of \flower
...
On the other hand, the MCs in the satellite galaxies are found to have
...
Given the low $\alpha_{\rm vir}$ parameter
and the $\sigma^2/R - \Sigma$ relation found for the MCs of the satellite galaxies, 
...
Red lines show the relation obtained from 
Literature data are compiled from \citet{Larson81a, Heyer04a, Rosolowsky05a, Bolatto08a, Swinbank11a, Leroy15a}.
...
Bottom: $\sigma^2/R - \Sigma_{\rm gas}$ relation of the MCs (star symbols) 
...
In both panels, the star symbols are color-coded by the $n_{\rm cut}$ adopted to identify the corresponding MCs.
\label{fig:alpha16}}
\end{figure*}
...
The range of masses of the MCs identified are comparable to those observed in \z$\sim$2 galaxies in $i$-band 
(i.e., rest-frame visible), which is between $M_{\rm cl}\approx$10$^{6.5-9xx}$\,\Msun \citep{Elmegreen07a, Elmegreen09a}.
Observational studies often report the mass distribution in the form of a cumulative function (CMF).
While the CMF of the MCs is quite noisy due to resolution effect and since we only have one galaxy here 
in the cosmological zoom-in simulation, we could improve the ``signal-to-noise ratio'' by including 
MCs identified from the different snapshots\footnote{This approach is valid unless in some snapshots,
...
The slope varies from $-$0.49\pmm0.01 to $-$1.17\pmm0.06 
for MCs identified using the lowest and the highest $n_{\rm cut}$ adopted in this study.
...
(see e.g., \citealt{Klessen10a, Goldaum11a}). % turbulence when disk still accreting gas
...
\AP{I would move it to the previous section; I would use the virial parameter analysis to discuss the gravitational instability point more than on evolutionary state of the galaxy}

We quantify how stable a molecular structure is using the virial parameter, which
is defined using \Eqn{virial} and describes the balance between kinetic and gravitational potential energies.
An $\alpha_{\rm vir}\approx$2 corresponds to approximately equipartition between the two energy terms 
and is often used to assess the boundedness of a given structure \citep[see e.g., ][]{Kauffmann17b}\footnote{Note that 
the true virial state/boundedness of a given structure also depends on the surface terms and the magnetic field (see \Eqn{virial} 
in \Sec{PVE})}.

The high $\alpha_{\rm vir}$ found in the majority of MCs of \flower suggest that they are not collapsing.
...
the influence on the dynamics of MCs due to continuous gas accretion from the surrounding IGM (see e.g., \citealt{Klessen10a, Goldaum11a}).
...
High resolution imaging of the molecular gas content in the first galaxies with ALMA and the Next Generation Very Large Array (ngVLA), on the other hand,
