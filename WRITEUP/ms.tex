...
\begin{equation}
c_s^2 = \frac{\sum_{i} \rho_i \left(P_{{\rm TH}, i}~{\rm k_B} / m_p / \rho_i \right)^2}{\sum_i \rho_i}\,,
...
%
Note that the bulk velocity dispersion $\sigma_{\rm bulk}$ is typically comparable to the non-thermal term $\sigma_{\rm NT}$, as the two are connected via the Euler equation \AP{do we want to add a plot here to show the point?}\DL{This is currently shown in \Fig{vv}}, but this depends on the evolutionary stage of \flower.
%
%\footnote{Velocity dispersions calculated from velocity field are comparable to those calculated from the pressure field indicating that contribution from shear is unlikely to dominate the velocity dispersions of the smaller MCCs identified, especially at the highest $n_{\rm ncut}$.\label{ftn:veldisp}}.
...
gas in equilibrium, see Equation~\ref{eqn:v0}. \AP{$\sigma$ should be $\sigma_{\rm gas}$}
...
relation down to 1\,pc are comparable  to those  of  the  cores  found  in regions  of high-mass star formation, suggesting that the identified MCCs are capable of high-mass \SF \AP{please change y label to $M_{\rm gas}$; note that in Fig. \ref{fig:dist} R instead of cloud size is used; we have to pick one}.
\label{fig:MR}}
%mm}
\end{figure*}
...
MCCs in the pre-starburst phase have a lower $\alpha_{\rm vir}$ compared to the starburst phase. \AP{please change x label to $M_{\rm gas}$; note that this plot seems a bit redundant}
\label{fig:alphaEvol}}
\end{figure*}
...
density thresholds (top right panel). \AP{please change x labels of the middle panels to $M_{\rm gas}$; $\sigma$ should be $\sigma_{\rm gas}$}
...
form the cluster of points that are to the top left of the parent MCC identified at a lower density threshold. \AP{$\rho \rightarrow n$; this should stay close to Fig. \ref{fig:MR}}
\label{fig:rho}}
\vspace{0.5em}
\end{figure}
