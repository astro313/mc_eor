...
kinetic and thermal energies. We find that ..
\end{abstract}
...
At high redshifts, galaxies have higher star formation rates (SFR) and smaller sizes {\bf cite needed?}, thus are expected to have more \ion{H}{2} regions and ionized gas, and more intense radiation stellar feedbacks. Since also the metallicity and dust content should be lower {\bf cite?}, this affects the shielding of UV photons, heating and cooling mechanisms. Such differences in turn can have a role in regulating the thermal and chemical structures of their ISM, and thus, their multi-phase ISM differ from nearby galaxies.
%
...
\AP{i moved the section here, as I see it as part of motivation for the workflow; however, at the moment I do not see the stellar mass maps as figure that are needed; if they stay i would label their evolutionary stage in the SFR history plot.}
...
found based on the Minkovsky analysis presented by \citet{Pallottini17b}.
...
$M_*$)).
% plt.hist($M_{\rm cl}$), color by two different mass range.. ~2E7?
% plt.hist($R_{\rm cl}$), using same color scheme
% for f_gas, same color scheme
...
% which may determine the ``cloud'' mass if these MCs are formed due to gravitational
% instabilities. We show gas fraction as a function of ``cloud'' mass in \Fig{}.
...
In \Sec{Q}, we also measure the contributions to the extracted velocity dispersions of the
...
to the top panel of \Fig{phases} and \Fig{larsons_single}), \flower has assembled
...
% \subsubsection{Single Snapshots: Size Mass Relation} to merge with section 3.2.1
