...
\Sec{singless}). The two phases correspond to (a) intense accretion
(b) starburst (\Fig{SFH}). We are interesting in determining whether
...
%mm [Figures must be numbered and placed in order cited.  Better to
%cite to the section and leave the figure in its proper place.] \Fig{alpha16-28}).
     Sect.~\ref{sec:ncut}).
...
indicating that contribution from shear is unlikely to dominate the velocity dispersions reported here.\label{ftn:veldisp}}. 
...
\citep{Elmegreen07a, Elmegreen09a}. The stellar-to-gas mass ratio ranges ($M_{\star}/M_{\rm cl}$) between $<$\,0.1 to 70 over all the
MCCs identified across all evolutionary stages of \flower.
...
We first focus on the two most extreme evolutionary stages of \flower---the accreting and starburst phases (see \Sec{sfh}), 
and identify a set of MCCs in each of them. \DL{note that delay in cloud properties and SFR.}
...
\citep{Rosolowsky05a, Swinbank11a}.
Since the sizes are somewhat dependent on the choice of $n_{\rm cut}$, as such, 
some of the MCCs within the main disk break into multiple sub-MCCs at higher $n_{\rm cut}$. 
...
% Starburst 
In the starburst phase, some MCCs have lower velocity dispersion, virial parameter, 
and $\sigma^2/R$ compared to the accreting phase. \DL{Is this significant enough to say something physical? If so, not immediately clear to me what cause the differences.}
% PVE
...
$\log{(P/\textrm{K cm}^{-3})}$\eq6, \MM{This is not shown in the
...
%
In the same plot, we also show loci of constant surface densities,
which are parameterized through the visual extinction $A_V$. \MM{Why
  do you go through $A_v$ rather than just using surface densities
  directly?} As shown
in \Fig{MR}, the MCCs identified in \flower lie above this relation,
along the locus of $A_v \simeq 8.9$ clouds observed by
\citet{Lombardi10a} in the MW. \MM{Does the variation in surface
  density seen in Figure 6 correspond to the variation in $A_V$ seen
  here?}
%
...
%Star symbols lying close to $\alpha_{\rm vir}\simeq2$ correspond to MCCs in the satellite galaxies.
%\label{fig:alpha16}}
%\end{figure*}
%
%\begin{figure*}[htbp]
...
MCCs in $z$\,\ssim2 galaxies could be smaller.
%mm in reality.
%
...
which is consistent with the high $\alpha_{\rm vir}$ found for the MCCs in the main disk of \flower.
