...
\begin{abstract}
We study the dynamical properties of molecular clouds and their temporal evolution in a prototypical galaxy 
at the Epoch of Reionization using state-of-the-art cosmological zoom-in simulation, 
...
We find that the MC of \flower are highly supersonic, with high velocity dispersions ($\sigma\approx$\,200\,\kms) that are comparable to 
those observed in gas-rich galaxies in the local universe as well as at the peak epoch of cosmic \SF. 
The mass scale of the MCs is of the order $10^{6.5-8}$\,\Msun. The $\sim$200\,pc-scale MCs found with a low density threshold 
correspond to the arms of the main disk of \flower and break down into smaller $\lesssim$100\,pc-scale MCs at higher density thresholds. 
The more massive and bigger MCs in \flower likely results from the higher gas mass fraction, surface density, and velocity dispersion, 
which sets the scales for fragmentation.
We compare the dynamics of MCs in \flower and those observed in the local Universe in the context of the Larson's relation.
The MCs of \flower are systematically found to have higher $\sigma$ and $\Sigma$ regardless of the $n_{\rm cut}$ adopted.
The higher velocity dispersion remains $\gtrsim$100\,\kms even when we increase the $n_{\rm cut}$ and even for the molecular substructures, likely 
because \flower has experienced more episodes of recent \SF, and thus stronger SNe and stellar feedback.
...
On the other hand, we find $\alpha_{\rm vir}$\ssim1 for MCs in the satellite galaxies. We interpret this as a result of the weaker stellar feedback as they have
experienced less episodes of \SF compared to \flower (also supported by the lower stellar-to-gas mass ratio of the latter).
MCs in the satellites are therefore likely collapsing structures. This paints a picture, where at the EoR, \SF continues as gas is being accretion from the IGM.
...
Higher resolution cosmological zoom-in simulations will ... shed light into ... , as we .... from observations with ALMA.
\end{abstract}
...
and thus, they are expected to have more \ion{H}{2} regions, ionized gas, and more intense radiation stellar feedbacks. 
...
%
% talk about dynamical states of MCs of different galaxies? Even in the local universe, where the most detailed \obs can be done, we see variations in cloud properties in different galaxy populations (see e.g., Hughes10a, MNRAS, 406, 2065; Hughes+13b, ApJ, 779, 46), maybe also Meidt+13, apj, 779,45?. Thus, obvious to pose the question what are the dynamical states of the MCs of the first galaxies and how do they differ from nearby universe, and analogous to any of the nearby \galpop?
% 
...
Briefly, by \z$\sim$6, \flower is a LBG hosted in a dark matter halo of mass $M_{\rm DM}\simeq$\,10$^{10}$\,\Msun at the center of a cosmic web knot, and accretes mass from the intergalactic medium mainly via three filaments of length $\simeq$\,100\,kpc. At \z$\sim$6, \flower has a stellar mass of $M_*\simeq$\,3\E{10}\,\Msun, metallicity of $Z\simeq$\,0.5\,$Z_{\odot}$, a molecular gas mass of $M_{\rm H2}\simeq$\,5\E{7}\,\Msun, and a globally-integrated SFR of $\simeq$\,100\,\Msun\,yr\pmOne. It is therefore a prototypical galaxy at \z$\sim$6.
...
actively accreting materials from its surrounding (top panel of \Fig{SFH})
and another of which \flower is undergoing
a starburst phase after a major merger (bottom panel of \Fig{SFH}).

\begin{figure*}[htbp]
\centering
\includegraphics[trim=10 0 0 0, clip, width=0.65\textwidth]{\figpath/daisyleung2.pdf}
\caption{
...
%
The latest version of the default \ncode{yt} clump finder decomposes the zones in the simulation into non-overlapping tiles, which are stored in a k-dimensional tree (aka k-d tree). It then identifies the contours of a variable field (e.g., density field) within a tile and connects them across the tiles. 
...
$n_{\rm cut}$\eq[0.32, 0.53, 0.88, 1.45, 2.45, 4.08, 6.81, 11.36, 19.00, 31.62]\,cm$^{-3}$.
	Color shows the projected H$_2$ surface density. {\bf this should be H2 density 1/cm$^{-2}$?!} \AP{if i recall correctly, this is not the projected h2 density, but the mass averaged h2 density field. in any case, we should compress the information, by having a less figures showing multplie cuts (e.g. 3 of them by using thinner lines should suffice in conveying the overview properly); additionally, we should convince yt to plot a proper label for the colorbar, and remove the trailing right axes labels (that are still there despite our efforts)}
...
molecular gas masses, radii, gas mass fraction ($sf_{\rm gas}$\eq$M_{\rm gas}$/($M_{\rm gas}$ +
$M_*$)).
...
and are not supported against collapse by external pressure. % are bound
...
with visual extinction of $A_V$\eq4\,mag. This may suggest that xyzxyz.
