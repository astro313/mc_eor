...
TODO
----
- color and label snapshot by their global SFRs from star particles instead.
...
xngc253, yngc253 = np.loadtxt(litpath + 'Leroy15_NGC253.csv', delimiter=',',
    unpack=True)
...
ax.scatter(xngc253, yngc253, label="NGC 253", color='red', marker='o', s=7)
...
ax.hist(allmasses, bins=200, normed=True)  # , cumulative=True)
...
plt.tight_layout()
plt.show()
fig.savefig(leafdir_out + 'MassDistributionPDF.png', bbox_inches="tight")


# --- do plots for the .p files in test_brute/ ---
# fname = "0.32_10_fields.p"
fname = "0.68_10_fields.p"
# fname = "14.68_10_fields.p"
# fname = "1.47_10_fields.p"
# fname = "3.16_10_fields.p"
# fname = "31.62_10_fields.p"
# fname = "35.00_10_fields.p"
# fname = "6.81_10_fields.p"


outleafdir = 'test_brute/leaf_fields_ss16-28/'
leafdir_out = outleafdir + fname[:fname.find('.p')] + '/'
if not os.path.isdir(leafdir_out):
...
ss = {}
for issnum in range(16, 29):
    leafdir = 'test_brute/leaf_fields_' + str(issnum) + '/'
    try:
        leaf_fields = pickle.load(open(leafdir + fname, "rb"))
    except:
        continue
...
ax.hist(allmasses, bins=200, normed=True)  # , cumulative=True), normed=True meaning y-axis shows < 1.0
...
mbin, df = mass_function(allmasses[allmasses<2.e7], logged=False, nbins=8)
...
plt.tight_layout()
plt.show()
fig.savefig(leafdir_out + 'MassDistribution_differential.png', bbox_inches="tight")
